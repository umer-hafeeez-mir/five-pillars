...
/* ---------------- Guided Flow Slide Transition (ONLY for guided step cards) ---------------- */

type SlideDir = "forward" | "back";

/**
 * Lightweight slide transition that:
 * - ONLY wraps guided-step card content
 * - Preserves height during animation (prevents layout jump)
    ...
    >
      ...
      {prevNode && animating ? (
        <div
          ...
          ref={(el) => (prevMeasureRef.current = el)}
      ...
      <div
        ...
        ref={(el) => (currMeasureRef.current = el)}
...
function EnterLayer({ from, duration, children }: { from: string; duration: number; children: React.ReactNode }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        transitionDuration: `${duration}ms`,
        transitionTimingFunction: "cubic-bezier(0.2, 0.8, 0.2, 1)",
        transform: on ? "translateX(0%)" : from,
        opacity: on ? 1 : 0
      }}
    >
      {children}
...
function ExitLayer({ to, duration, children }: { to: string; duration: number; children: React.ReactNode }) {
...
export default function Page() {
  ...
  const [lastFetchedAt, setLastFetchedAt] = usePersistedState<number | null>("fp_rates_last_fetched_v1", null);

  // Power-user accordion open state (kept)
  const [openSection, setOpenSection] = usePersistedState<ZakatSection>("fp_zakat_open_section_v2", "nisab");
  ...
  const estimatedNisab = zakatResult && zakatResult.nisab > 0 ? `₹ ${formatINR(zakatResult.nisab)}` : "₹ —";
  ...
  /* ---------------- Guided direction tracking (ONLY for guided steps) ---------------- */
  const prevGuidedStepRef = useRef<GuidedStep>(guidedStep);
  const guidedDir: SlideDir = guidedStep >= prevGuidedStepRef.current ? "forward" : "back";
  useEffect(() => {
    prevGuidedStepRef.current = guidedStep;
  }, [guidedStep]);
          ...
          <>
            ...
            {showSummary ? (
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-6 soft-shadow">
                  ...
                  <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="text-sm text-slate-600">{zakatResult?.eligible ? "Zakat to pay" : "Zakat (not due)"}</div>
                      ...
                      >
                        {zakatResult?.breakdown?.nisabRateMissing ? "Needs rate" : zakatResult?.eligible ? "Due" : "Not due"}
                      </span>
                    </div>
              ...
              <div className="mt-6 max-w-3xl mx-auto space-y-4">
                <CollapsibleCard title="Nisab & Eligibility" subtitle={nisabSubtitle} open={openSection === "nisab"} onToggle={() => toggleSection("nisab")}>
                    ...
                    <div className="mt-4">
                      ...
                      <div className="mt-3 flex items-center justify-between gap-3">
                        <div className="text-xs text-slate-500">
                          Last updated: <span className="font-medium text-slate-700">{formatDateTime(lastFetchedAt)}</span>
                        </div>
                ...
                <CollapsibleCard title="Cash & Bank" subtitle={cashSubtitle} open={openSection === "cash"} onToggle={() => toggleSection("cash")}>
                  <div className="space-y-3">
                    <Field label="Cash in hand" hint="Money you currently have available." prefix="₹" value={z.cash} onChange={(v) => setZ((s: any) => ({ ...s, cash: v }))} />
                    <Field label="Cash in bank" hint="Total balance across your bank accounts" prefix="₹" value={z.bank} onChange={(v) => setZ((s: any) => ({ ...s, bank: v }))} />
                  </div>
                </CollapsibleCard>

                <CollapsibleCard title="Gold & Silver" subtitle={metalsSubtitle} open={openSection === "metals"} onToggle={() => toggleSection("metals")}>
                  <div className="space-y-3">
                    <div>
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white p-3">
                      ...
                      <div className="mt-3 space-y-3">
                        <Field label="Gold (grams)" hint="Weight of gold you own for this purity." suffix="g" value={(activeHolding as any).grams ?? ""} onChange={(v) => updateHolding(activeKarat, { grams: v })} />
                        <Field label="Gold rate (₹/g)" hint="Current market price per gram for this purity." prefix="₹" value={(activeHolding as any).rate ?? ""} onChange={(v) => updateHolding(activeKarat, { rate: v })} />
                      </div>
                    ...
                    <Field label="Silver (grams)" hint="Weight of silver you own." suffix="g" value={z.silverGrams} onChange={(v) => setZ((s: any) => ({ ...s, silverGrams: v }))} />
                    <Field label="Silver rate (₹/g)" hint="Current market price per gram." prefix="₹" value={z.silverRate} onChange={(v) => setZ((s: any) => ({ ...s, silverRate: v }))} />
                  </div>
                </CollapsibleCard>

                <CollapsibleCard title="Other Assets" subtitle={otherSubtitle} open={openSection === "other"} onToggle={() => toggleSection("other")}>
                  <div className="space-y-3">
                    <Field label="Investments / savings" hint="Stocks, mutual funds, savings plans, etc." prefix="₹" value={z.investments} onChange={(v) => setZ((s: any) => ({ ...s, investments: v }))} />
                    <Field label="Business assets" hint="Inventory, goods held for sale, business cash, receivables." prefix="₹" value={z.businessAssets} onChange={(v) => setZ((s: any) => ({ ...s, businessAssets: v }))} />
                    <Field label="Money lent to others" hint="Money you expect to receive back." prefix="₹" value={z.moneyLent} onChange={(v) => setZ((s: any) => ({ ...s, moneyLent: v }))} />
                  </div>
                </CollapsibleCard>

                <CollapsibleCard title="Deductions" subtitle={deductionsSubtitle} open={openSection === "deductions"} onToggle={() => toggleSection("deductions")}>
                  <div className="space-y-3">
                    <Field label="Debts & liabilities" hint="Bills or loans you must repay soon." prefix="₹" value={z.debts} onChange={(v) => setZ((s: any) => ({ ...s, debts: v }))} />
                  </div>
                </CollapsibleCard>
                ...
                <BottomBar>
                  <button
                    ...
                    className={[
                      ...
                      !isRateValid ? "bg-slate-200 text-slate-500 cursor-not-allowed" : "bg-emerald-800 hover:bg-emerald-900 text-white"
                    ].join(" ")}
                  >
                    Calculate Zakat
                  </button>
              ...
              /* ================= GUIDED FLOW ================= */
              <div className="mt-6 max-w-3xl mx-auto space-y-4">
                {/* ✅ ONLY the step card slides; bottom bar stays stable */}
                <GuidedStepSlide stepKey={guidedStep} direction={guidedDir}>
                  <>
                    {/* Intro card */}
                    {guidedStep === 0 ? (
                      <div className="rounded-2xl border border-slate-200 bg-white p-6 soft-shadow">
                        <div className="text-[11px] tracking-widest text-slate-500 font-semibold">GUIDED FLOW</div>
                        <div className="mt-2 text-xl font-semibold text-slate-900">Let’s calculate your Zakat step-by-step</div>
                      ...
                      <div className="rounded-2xl border border-slate-200 bg-white p-6 soft-shadow">
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-3">
                          <button
                            ...
                            className={[
                              "rounded-xl border px-4 py-3 text-sm font-semibold transition",
                              ownsGold ? "border-emerald-300 bg-emerald-50 text-emerald-900" : "border-slate-200 bg-white text-slate-800 hover:bg-slate-50"
                            ].join(" ")}
                          >
                            Yes
                          </button>
...
function Row({
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      <div className={[strong ? "font-semibold text-slate-900" : "text-slate-700", muted ? "text-slate-500" : ""].join(" ")}>
        {label}
      </div>
...
function BottomBar({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40">
      <div className="container-page pb-4" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 12px)" }}>
