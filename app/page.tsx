...
function SlideTransition({
  ...
  const [currKey, setCurrKey] = useState<string | number>(transitionKey);
  ...
  // If content changes without key change, keep it updated (no animation)
  useEffect(() => {
    if (transitionKey === currKey) setCurrNode(children);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [children]);

  useEffect(() => {
    if (transitionKey === currKey) return;

    setPrevNode(currNode);
    setPrevKey(currKey);

    setCurrNode(children);
    setCurrKey(transitionKey);
  ...
  const enterFrom = direction === "forward" ? "translateX(12%)" : "translateX(-12%)";
  const exitTo = direction === "forward" ? "translateX(-12%)" : "translateX(12%)";

  return (
    <div className="relative overflow-hidden motion-reduce:overflow-visible">
      {/* previous panel (slides out) */}
      {prevNode && animating ? (
        <SlideOut key={`prev-${String(prevKey)}`} duration={DURATION} to={exitTo}>
          {prevNode}
        </SlideOut>
      ) : null}

      {/* current panel (slides in) */}
      <SlideIn key={`curr-${String(currKey)}`} duration={DURATION} from={enterFrom}>
        {currNode}
      </SlideIn>

      {/* height stabilizer while animating */}
      {animating ? <div aria-hidden className="invisible">{currNode}</div> : null}
    </div>
  );
}
...
export default function Page() {
  ...
  }>({ showSummary, zakatMode, guidedStep });

  const contentKey = showSummary
    ? "summary"
    : zakatMode === "power"
    ? "power"
    : `guided-${guidedStep}`;

  let contentDir: SlideDir = "forward";
  const prev = prevStateRef.current;

  if (showSummary !== prev.showSummary) {
    contentDir = showSummary ? "forward" : "back";
  } else if (zakatMode !== prev.zakatMode) {
    contentDir = zakatMode === "power" ? "forward" : "back";
  } else if (guidedStep !== prev.guidedStep) {
    contentDir = guidedStep > prev.guidedStep ? "forward" : "back";
  }

  useEffect(() => {
    prevStateRef.current = { showSummary, zakatMode, guidedStep };
  }, [showSummary, zakatMode, guidedStep]);
      ...
      <section className="container-page pb-24">
        ...
        {active !== "zakat" ? (
          ...
          <>
            ...
            {/* MAIN CONTENT (slides horizontally for Next/Back + mode + summary) */}
            <div className="mt-6 max-w-3xl mx-auto">
              <SlideTransition transitionKey={contentKey} direction={contentDir}>
                <>
                  {/* ================= SUMMARY ================= */}
                  {showSummary ? (
                    <div>
                        ...
                        <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                          ...
                          <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
                            ...
                            <div className="mt-3 space-y-2 text-sm">
                              ...
                              <Row
                                label="Gold (24K)"
                                value={`₹ ${formatINR(n(h["24k"].grams) * n(h["24k"].rate))}`}
                              />
                              <Row
                                label="Gold (22K)"
                                value={`₹ ${formatINR(n(h["22k"].grams) * n(h["22k"].rate))}`}
                              />
                              <Row
                                label="Gold (18K)"
                                value={`₹ ${formatINR(n(h["18k"].grams) * n(h["18k"].rate))}`}
                              />
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="rounded-xl border border-slate-200 bg-white p-3">
                            ...
                            <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
                              Tip: Select the gold purity (24K, 22K, or 18K), then enter the grams and rate for the
                              selected purity.
                            </div>
                          </div>
                      ...
                      <Accordion title="How Zakat is calculated">
                        <div className="text-sm text-slate-600 leading-relaxed space-y-2">
                          ...
                          <p className="text-sm">
                            <b>Net</b> = (Cash + Bank + Gold value + Silver value + Investments + Business assets + Money
                            lent) − Debts
                          </p>
                          <p>
                    ...
                    <div className="space-y-4">
                      ...
                      {guidedStep === 2 ? (
                        <div className="rounded-2xl border border-slate-200 bg-white p-6 soft-shadow">
                          ...
                          <div className="mt-1 text-sm text-slate-600">
                            This is needed to calculate the Nisab threshold.
                          </div>
                      ...
                      {guidedStep === 3 ? (
                        <div className="rounded-2xl border border-slate-200 bg-white p-6 soft-shadow">
                          ...
                          <div className="mt-2 text-base font-semibold text-slate-900">
                            3) Add Cash in Hand and bank
                          </div>
                          <div className="mt-1 text-sm text-slate-600">
                            Add the money you currently have available including cash in hand and funds in your bank
                            accounts.
                          </div>
