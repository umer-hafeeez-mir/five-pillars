...
type GuidedStep = 0 | 1 | 2 | 3 | 4 | 5 | 6; // 0 intro, 1 basis, 2 rate, 3 cash, 4 gold/silver, 5 other, 6 deductions -> then summary
...
export default function Page() {
  ...
  const [guidedStep, setGuidedStep] = usePersistedState<GuidedStep>("fp_guided_step_v1", 0);
  ...
  // Keep the “first open is guided intro” behavior (only for zakat tab)
  useEffect(() => {
    if (active !== "zakat") return;
    if (zakatMode === "guided" && guidedStep === 0 && showSummary) {
      // if somehow summary persisted, keep it; otherwise remain intro
      return;
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [active]);
  ...
  // Derived values (same as earlier)
          ...
          <>
              ...
              <div className="inline-flex rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
                <button
                  ...
                  className={[
                    ...
                    zakatMode === "guided" ? "bg-emerald-50 text-emerald-900 border border-emerald-200" : "text-slate-700"
                  ].join(" ")}
                >
                  Guided flow
                </button>
                <button
                  ...
                  className={[
                    "px-4 py-2 text-sm font-semibold rounded-lg transition",
                    zakatMode === "power" ? "bg-emerald-50 text-emerald-900 border border-emerald-200" : "text-slate-700"
                  ].join(" ")}
                >
                  Power users
                </button>
            ...
            {showSummary ? (
              <div className="mt-6 max-w-3xl mx-auto">
                    ...
                    <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
                      ...
                      <div className="mt-3 space-y-2 text-sm">
                        ...
                        <Row
                          ...
                          value={`₹ ${formatINR(zakatResult?.net ?? 0)}`}
                          strong
                        />
                      </div>
                    </div>
                  ...
                  <div className="mt-5 flex items-center justify-between gap-3">
                    ...
                    <button
                      ...
                      onClick={() => {
                        setShowSummary(false);
                        if (zakatMode === "guided") {
                          // send them back into guided steps (start at step 1)
                          setGuidedStep((s) => (s === 0 ? 1 : s));
                        }
                      }}
                ...
                {/* Sticky buttons for BOTH modes (below summary): Share + Reset */}
                <BottomBar>
                  <button
                    type="button"
                    onClick={handleShare}
              ...
              <div className="mt-6 max-w-3xl mx-auto space-y-4">
                ...
                <BottomBar>
                  <button
                    ...
                    className="w-full rounded-xl bg-emerald-800 hover:bg-emerald-900 text-white py-3 font-semibold transition"
                  >
                    Calculate Zakat
                  </button>
                  <button
              ...
              /* ================= GUIDED FLOW ================= */
              <div className="mt-6 max-w-3xl mx-auto space-y-4">
                {/* Intro card: disappears after user starts (step 0 -> 1) */}
                {guidedStep === 0 ? (
                  <div className="rounded-2xl border border-slate-200 bg-white p-6 soft-shadow">
                ...
                {guidedStep === 1 ? (
                  <div className="rounded-2xl border border-slate-200 bg-white p-6 soft-shadow">
                    ...
                    <div className="mt-1 text-sm text-slate-600">
                      Do you want to calculate using silver or gold?
                    </div>
                ...
                {guidedStep === 4 ? (
                  <div className="rounded-2xl border border-slate-200 bg-white p-6 soft-shadow">
                    ...
                    <div className="mt-1 text-sm text-slate-600">
                      Add the grams you own and the per gram rate.
                    </div>
                ...
                {/* Step 6: Deductions + Calculate */}
                {guidedStep === 6 ? (
                  <div className="rounded-2xl border border-slate-200 bg-white p-6 soft-shadow">
                    <div className="text-[11px] tracking-widest text-slate-500 font-semibold">GUIDED FLOW</div>
                    <div className="mt-2 text-base font-semibold text-slate-900">6) Deductions</div>
